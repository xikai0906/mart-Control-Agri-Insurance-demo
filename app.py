...
st.markdown("""
    ...
    .subsidy-highlight {
        background: linear-gradient(90deg, #166534, #1E3A8A);
        color: white;
        padding: 1rem;
        border-radius: 10px;
        text-align: center;
...
with nav_col5:
    if st.button("🏛️ 政府公益端", use_container_width=True, type="secondary"):
        st.switch_page("pages/5_政府公益端.py")
...
with col1:
    ...
    <div class="feature-box">
        <h3>🌡️ 天气理赔</h3>
        <p>AI实时监测极端天气，自动触发理赔预警，80%政府补贴保障农业生产安全</p>
    </div>
    """, unsafe_allow_html=True)
with col2:
    st.markdown("""
    <div class="feature-box">
        <h3>💰 价格理赔</h3>
        <p>智能预测农产品价格波动，价格低于保险阈值自动理赔，公益补贴助力小农户</p>
    </div>
    """, unsafe_allow_html=True)
with col3:
    st.markdown("""
    <div class="feature-box">
        <h3>🤖 AI风控</h3>
        <p>多维数据交叉验证，提升理赔效率，防范虚假理赔风险，服务乡村振兴</p>
    </div>
    """, unsafe_allow_html=True)
...
2. 🔍 探索各项功能模块（每笔保费均显示80%政府公益补贴）
3. 💡 体验AI智能服务 + 政策性农险透明机制
**Demo说明:** 本系统为演示版本,部分数据为模拟生成,体现政策性农业保险（中央+地方财政补贴80%以上）公益属性，仅供参考学习
""")
