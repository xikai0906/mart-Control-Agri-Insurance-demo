...
if image_path.exists():
    ...
    .stApp::before {{
        ...
        background: rgba(0, 0, 0, 0.78);  /* 加深到78% 让文字更清晰 */
        z-index: -1;
    }}
    h1, h2, h3 {{
        color: #00bfff !important;        /* 标题用亮蓝色（政府正式感） */
        text-shadow: 0 3px 6px rgba(0,0,0,0.9);
    }}
    p, li, .stMarkdown, .stSuccess, .stError, .stInfo, .stWarning, 
    .stMetricLabel, .stDataFrame, .stCaption {{
        color: #00cc66 !important;        /* 正文用翠绿色（农业/乡村振兴主题） */
...
st.success("✅ 背景图 + 绿色/蓝色/红色字体已更新！文字清晰度大幅提升")
